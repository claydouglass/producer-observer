      ...
      {expanded && byProduct && (
        <div className="mt-4">
          ...
          <div className="mt-4 p-3 bg-indigo-50 rounded-lg text-sm text-indigo-800">
            Based on current velocity, plan production for{" "}
            <strong>
              {byProduct
                .reduce((sum, p) => sum + p.forecast180d, 0)
                .toLocaleString()}{" "}
              units
            </strong>{" "}
            over the next 6 months. Start flower now for Q3 delivery.
          </div>
        </div>
      )}
    </div>
